 ...
 * Process main product image - WITH CREDIT DEDUCTION
 */
export async function POST(request) {
  try {
    ...
    if (productError || !product || product.user_id !== user.id) {
      return NextResponse.json(
        { error: 'Unauthorized to process this product' },
        { status: 403 }
      );
    }
    ...
    if (profile) {
      const newCreditsRemaining = profile.credits_remaining - imageCount;
      const newTotalUsed = profile.total_credits_used + imageCount;

      await supabase
        .from('users')
        .update({
          credits_remaining: Math.max(0, newCreditsRemaining),
          total_credits_used: newTotalUsed,
        })
        .eq('id', user.id);

      console.log('Credits deducted:', imageCount, 'New balance:', newCreditsRemaining);
    }
